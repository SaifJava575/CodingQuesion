...
public class Hd_46_MakingLargestIslands {
	...
	static int dfs(int i, int j, int[][] grid, boolean[][] vis, int name, int N) {
		...
		vis[i][j] = true;
		grid[i][j] = name;
		int count = 1 + dfs(i - 1, j, grid, vis, name, N) + dfs(i + 1, j, grid, vis, name, N)
				+ dfs(i, j - 1, grid, vis, name, N) + dfs(i, j + 1, grid, vis, name, N);
		return count;
	}
	...
	public static void main(String[] args) {
		...
		int response = largestIsland(N, grid);
		System.out.println("The largest islands are ::" + response);
